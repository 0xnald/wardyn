...
const commandSchema = z.discriminatedUnion('command', [
  z.object({ command: z.literal('scenario'), scenario: z.enum(['balanced','concentration','drawdown','deterioration','reserve']) }),
...
function response(value: unknown, id: string, status = 200) {
  ...
  result.cookies.set('wardyn-session', id, { httpOnly: true, sameSite: 'strict', secure: process.env.NODE_ENV === 'production', path: '/', maxAge: 604800 });
  return result;
}
export async function GET(request: NextRequest) {
  const id = session(request);
  try { return response(await store.update(id, async s => s), id); }
  catch { return response({ error: 'Could not load the saved Wardyn session.' }, id, 500); }
}
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (command.command === 'interpret') {
      const now = Date.now();
      if (now - (lastAiCall.get(id) ?? 0) < 5000) return response({ error: 'Wait a few seconds before generating another draft.' }, id, 429);
    ...
    const result = await store.update(id, async state => {
      switch (command.command) {
        ...
        case 'policy': await activatePolicy(state, command.policy); break;
        case 'action': await action(state, command.receiptId, command.approve); break;
        case 'monitor': state.monitoring = command.enabled; state.revision += 1; break;
        case 'connect': await connectAccount(state); break;
      }
      return state;
    });
    return response(result, id);
  } catch (error) {
    const message = error instanceof z.ZodError ? 'Some values are invalid or outside policy bounds.' : error instanceof Error ? error.message : 'The request failed.';
