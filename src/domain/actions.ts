...
export function simulateSale(
  ...
  if (!quantity.isFinite() || quantity.lte(0) || quantity.gt(position.quantity))
    throw new Error('Invalid sale quantity');
  if (
    quantity
      .div(position.quantity)
      .mul(100)
      .gt(policy.maxActionPct + 0.000001)
  )
    throw new Error('Action size exceeds policy');
  ...
  if (!stable) {
    stable = { asset: 'USDT', quantity: '0', entryPriceUsdt: undefined, peakPriceUsdt: undefined };
...
export function createReceipt(
  ...
  return {
    ...
    expectedAfter:
      decision.trade && before.source === 'demo'
        ? simulateSale(before, decision.trade, policy, now)
        : null,
    after: null,
    status: decision.trade ? 'PENDING' : 'HOLD',
    verified: null,
    verification: null,
  };
}
...
export function approveReceipt(
  ...
  if (receipt.status !== 'PENDING' || !receipt.decision.trade)
    throw new Error('This action is no longer pending');
  if (now.getTime() - Date.parse(receipt.createdAt) > 120000)
    throw new Error('Proposal expired. Run a new scan.');
  if (JSON.stringify(current.positions) !== JSON.stringify(receipt.before.positions))
    throw new Error('Portfolio changed. Run a new scan.');
  const after = simulateSale(current, receipt.decision.trade, policy, now);
  const remaining = evaluatePolicy(after, policy);
  const unresolved = receipt.decision.triggers.filter((trigger) =>
    remaining.some((v) => v.asset === trigger.asset && v.rule === trigger.rule),
  );
  ...
  return {
    ...
    status: 'SIMULATED',
    after,
    verified: actionable.length === 0,
    verification: actionable.length
      ? 'Action simulated; some triggered limits remain breached. Cooldown prevents immediate repeated execution.'
      : 'Action simulated and triggered allocation / reserve / loss rules resolved. Profit scaling records a partial realization, not a lower return percentage.',
  };
}
