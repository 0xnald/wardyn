...
export type PolicyViolation = {
  ...
  rule: 'concentration' | 'reserve' | 'profit' | 'drawdown';
  actualPct: number;
  limitPct: number;
  explanation: string;
};
...
export type WardynDecision = {
  id: string;
  asset: Asset | 'PORTFOLIO';
  decision: DecisionKind;
  confidence: 'RULE_CONFIRMED' | 'INSUFFICIENT_DATA';
  reasons: string[];
  evidence: DecisionEvidence[];
  triggers: PolicyViolation[];
  trade: TradeIntent | null;
  approvalRequired: boolean;
  cooldown: boolean;
};
export type WardynReceipt = {
  id: string;
  createdAt: string;
  resolvedAt?: string;
