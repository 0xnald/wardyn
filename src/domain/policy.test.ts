...
test('policy bounds reject unrestricted execution and malformed numbers', () => {
  ...
  expect(policySchema.safeParse({ ...defaultPolicy, executionMode: 'automatic' }).success).toBe(false);
  expect(policySchema.safeParse({ ...defaultPolicy, maxAssetAllocationPct: 101 }).success).toBe(false);
