...
test('local parser handles the demo mandate without claiming AI', () => {
  const draft = fallbackPolicy(
    'Keep at least 15% in stablecoins, never let one asset exceed 30% of my portfolio, and take profits gradually.',
  );
...
test('invalid or unavailable model output cannot change execution authority', async () => {
  vi.stubEnv('WARDYN_AI_API_KEY', 'test-only');
  vi.stubEnv('WARDYN_AI_MODEL', 'test-model');
          ...
          content: [
            {
              type: 'tool_use',
              name: 'draft_policy',
              input: { policy: { executionMode: 'automatic' } },
            },
          ],
        }),
      ),
