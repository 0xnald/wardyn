...
test('policy edits supersede old approvals', async () => {
  const state = await initialState(); await runScenario(state, 'concentration');
  const id = state.receipts.find(r => r.status === 'PENDING')!.id;
  await activatePolicy(state, { ...state.policy, maxAssetAllocationPct: 40, minStableReservePct: 10 });
