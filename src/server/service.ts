...
export async function scan(state: WardynState) {
  ...
  if (state.portfolio.source === 'binance-cli')
...
export async function connectAccount(state: WardynState) {
  invalidatePending(state);
  record(state, await observe(new BinanceCliProvider(), state.policy, []));
}
